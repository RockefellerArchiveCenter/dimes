...
const PageDigitalObject = ({isMobile}) => {
  ...
  useEffect(() => {
    axios
      .get(`${import.meta.env.VITE_ARGO_BASEURL}/${type}/${id}`)
      .then(res =>  {
  ...
  const ViewerNavBar = (props) => {
    ...
    const imageDownloadUrl = infoResponse => {
      ...
      const downloadUrl = `${import.meta.env.VITE_S3_BASEURL}/images/${imagePath}?response-content-disposition=attachment;filename=${imagePath}.jp2`
      return downloadUrl
    }
